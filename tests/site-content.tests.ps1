...
if ($failures.Count -eq 0) {
    ...
    Assert-Contains $all "Copy Request" "booking fallback"
    Assert-Contains $all "0892721358" "WhatsApp contact number"
    Assert-Contains $all "copy your request and send it to us on WhatsApp" "WhatsApp booking instructions"
    Assert-Contains $all "Request ready to copy" "copy request success"
    ...
    Assert-Contains $all "buildWeekendOptions" "weekend-only date options"
    Assert-Contains $all "formatDateValue" "local date formatting"
    Assert-Contains $all "Select an available weekend" "weekend date label"
